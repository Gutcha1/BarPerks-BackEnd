"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createProductService = void 0;
const data_source_1 = require("../../data-source");
const entities_1 = require("../../entities");
const products_schemas_1 = require("../../schemas/products.schemas");
const errors_1 = require("../../errors");
const createProductService = (productData, pubId) => __awaiter(void 0, void 0, void 0, function* () {
    const productRepository = data_source_1.AppDataSource.getRepository(entities_1.Product);
    const pubRepository = data_source_1.AppDataSource.getRepository(entities_1.Pub);
    const pub = yield pubRepository.findOneBy({
        id: pubId
    });
    if (!pub) {
        throw new errors_1.AppError('Bar não encontrado', 404);
    }
    const data = Object.assign(Object.assign({}, productData), { pub: pub });
    const product = productRepository.create(data);
    yield productRepository.save(product);
    const productResponse = products_schemas_1.productsSchemaResponse.parse(product);
    return productResponse;
});
exports.createProductService = createProductService;
